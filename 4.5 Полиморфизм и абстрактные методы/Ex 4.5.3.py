class Student:
    def __init__(self, fio, group):
        ...
        self._house_marks = []  # оценки за домашнее задание
...
class Lector(Mentor):
    ...
    def set_mark(self, student, marks):
        student.add_lect_marks(marks)
...
class Reviewer(Mentor):
    ...
    def set_mark(self, student, marks):
        student.add_house_marks(marks)

    def __str__(self):
        return f"Эксперт {self._fio}: предмет {self._subject}"


lector = Lector("Балакирев", "Информатика")
reviewer = Reviewer("Гейтс", "Информатика")
students = [Student("Иванов", "ЭВМд-11"), Student("Гаврилов", "ЭВМд-11")]
